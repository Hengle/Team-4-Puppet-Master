...
namespace FPSBuilder.Core.Player
{
    ...
    public sealed class FirstPersonCharacterController : MonoBehaviour
    {
        ...
        private float m_CharacterWeight = 80;
        ...
        private float WalkingForce
        {
            get
            {
                if (LowerBodyDamaged)
                {
                    return m_WalkingForce * 0.7f;
                }

                return m_WalkingForce * WeightFactor;
            }
        }
        ...
        private void Start()
        {
            ...
            m_RigidBody.mass = m_CharacterWeight / 8;
            ...
            // Axes and buttons
            m_HorizontalAxis = InputManager.FindAxis("Horizontal");
            m_VerticalAxis = InputManager.FindAxis("Vertical");
            m_JumpButton = InputManager.FindButton("Jump");
            m_CrouchButton = InputManager.FindButton("Crouch");
            ...
            InvokeRepeating(nameof(CheckObstaclesAhead), 0, 0.1f);
        ...
        private void HandleInput()
        {
            ...
            {
                bool requestJump = InputManager.GetButtonDown(m_JumpButton);
            ...
            if (Grounded || State == MotionState.Climbing)
            {
                ...
                if (m_AllowCrouch)
                {
                    ...
                    {
                        if (InputManager.GetButtonDown(m_CrouchButton) && !IsCrouched && !m_Running && State != MotionState.Climbing && !IsSliding)
                        {
                            IsCrouched = true;
                        }
                        else
                        {
                            IsCrouched &= !m_Running && !IsSliding && State != MotionState.Climbing && !InputManager.GetButtonDown(m_CrouchButton) && !InputManager.GetButtonDown(m_JumpButton) || PreventStandingInLowHeadroom(transform.position);
                        }
                    }
                    else
                    {
                        ...
                        {
                            IsCrouched = InputManager.GetButton(m_CrouchButton);
                        }
                    }
                }
                ...
                if (InputManager.GetButtonDown(m_CrouchButton) && State == MotionState.Running && !IsSliding && !LowerBodyDamaged && m_StaminaAmount > m_MaxStaminaAmount * 0.4f
        ...
        private void CheckObstaclesAhead()
        {
            ...
            if (Physics.SphereCast(transform.position + Vector3.up * (m_Capsule.height * 0.05f), m_Capsule.radius * 0.75f, transform.TransformDirection(Vector3.forward),
                out RaycastHit hitInfo, m_Capsule.radius, m_AffectedLayers, QueryTriggerInteraction.Ignore))
            {
                if (hitInfo.collider.GetComponent<Rigidbody>())
                    return;

                // Analyze the normal vector and the obstacle height
                float angle = Vector3.Angle(transform.up, hitInfo.normal);
                Transform t = transform;
                CanVault = hitInfo.collider != null && angle > 85 && angle < 100 && IsValidObstacle() && !PreventStandingInLowHeadroom(t.position + t.forward);

                if (CanVault)
                {
                    Vector3 pos = t.position + new Vector3(0, m_CharacterHeight * 0.6f, 0) + t.forward * (m_Capsule.radius * 2);
                    Physics.Raycast(new Ray(pos, transform.TransformDirection(Vector3.down)), out RaycastHit obstacleInfo, m_CharacterHeight, m_AffectedLayers, QueryTriggerInteraction.Ignore);
        ...
        private bool IsValidObstacle()
        {
            ...
            Ray ray = new Ray(position + new Vector3(0, m_CharacterHeight * 0.5f, 0), transform.TransformDirection(Vector3.forward));
            bool isBlocked = Physics.Raycast(ray, radius * 2, m_AffectedLayers, QueryTriggerInteraction.Ignore);

            return !isBlocked && !Physics.SphereCast(ray, radius, out _, radius * 1.5f, m_AffectedLayers, QueryTriggerInteraction.Ignore);
        }
        ...
        private void ApplyGravityAndJumping(Vector2 input)
        {
            if (Grounded || m_Climbing)
            {
                ...
                if (m_Jump)
                {
                    ...
                    m_RigidBody.AddForce(new Vector3(0f, JumpForce * 10, 0f), ForceMode.Impulse);
                    m_Jumping = true;
            ...
            else
            {
                ...
                if (m_RigidBody.velocity.magnitude < Mathf.Abs(Physics.gravity.y * m_CharacterWeight / 2))
                {
        ...
        public Vector2 GetInput()
        {
            ...
            {
                x = InputManager.GetAxis(m_HorizontalAxis),
                y = InputManager.GetAxis(m_VerticalAxis)
            };
        ...
        private void CheckRunning()
        {
            if (!LowerBodyDamaged && GetInput().y > 0 && !IsAiming && !m_Climbing)
            {
                ...
                {
                    m_Running = InputManager.GetButton(m_RunButton);
                }
                else
                {
        ...
        private void PlayJumpSound()
        {
            ...
            m_NextStep = jump.length + Time.time;
        }
        ...
        private void PlayLandingSound(float fallDamage)
        {
            ...
            m_PlayerFootstepsSource.ForcePlay(land, m_LandingVolume);
